...
// ** Extended type for mock users with password
type MockUserDataType = UserDataType & {
  password?: string
  username: string
  fullName: string
}

const users: MockUserDataType[] = [
  {
    id: 1,
    role: 'admin',
    password: 'admin',
...
mock.onPost('/jwt/register').reply(request => {
  if (request.data.length > 0) {
    ...
    if (!error.username && !error.email) {
      ...
      const userData: MockUserDataType = {
        id: lastIndex + 1,
        email,
        password,
        username,
...
mock.onGet('/auth/me').reply(config => {
  ...
  jwt.verify(token, jwtConfig.secret as string, (err, decoded) => {
    ...
    if (err) {
      ...
      // ** Get user that matches id in token
      const userData = JSON.parse(JSON.stringify(users.find((u: MockUserDataType) => u.id === userId)))
